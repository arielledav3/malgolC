...
public class CodeGenerationVisitor implements ASTVisitor {
	...
	public void visit(Program p) {
		...
		*/
		...
		buf.append(generateInstruction(".ascii", "\"%d\\n\\0\""));
		buf.append(generateInstruction(".text"));
		buf.append(generateInstruction(".global_main"));
	...
	public void visit(FunctionDefinition d) {
		...
		//get location of the table that has frame size and variable
		...
		for(Declaration decl : d.getParameters()){
			...
			int local = currentLocationTable.lookup(name);
			Symbol symbol = Symbol.newVariableSymbol(name, type, true,local);
	...
	public void visit(FunctionCallExpression e) {
		...
		for(Expression expr : e.getArguments()){
			expr.accept(this);
			buf.append(generateInstruction("movl", "%eax", count + "(%esp)"));
			count += 4; //for add to frame space
		} //end loop
