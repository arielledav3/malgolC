...
public class ParseTree2ASTVisitor implements Analysis {
	...
	public void caseABlockBlock(ABlockBlock node) {
		PDeclList declList = node.getDeclList();
		declList.apply(this);
		LinkedList<Declaration> temp = declarationListResult;
		PStmtList stmtList = node.getStmtList();
		stmtList.apply(this);
		statementResult = new BlockStatement(node.getLBrace(), temp, statementListResult);
	}
	...
	@Override
	public void caseASomeDeclList(ASomeDeclList node) {
	...
	@Override
	public void caseANoneDeclList(ANoneDeclList node) {
